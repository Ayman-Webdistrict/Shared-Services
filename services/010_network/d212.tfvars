# ----------------------
# NETWORK SERVICE TFVARS
# ----------------------

# --------------------------------
# AWS ENVIRONMENT VARIABLES VALUES
# --------------------------------

aws_region   = "us-east-1"
aws_role_arn = "arn:aws:iam::844564028558:role/Admin"
aws_profile  = "fevo-dev"

# -----------
# VPC DETAILS
# -----------

environment          = "dev"
vpc_name             = "D212"
vpc_cidr             = ["172.16.23.0/24", "172.17.23.0/24", "172.18.23.0/24", "172.23.0.0/16", "172.19.0.0/16"]
vpc_azs              = ["us-east-1c", "us-east-1d"]
eks_cidr             = "172.23.0.0/16"
vpc_web_subnets      = ["172.16.23.0/25", "172.16.23.128/25"]
vpc_api_subnets      = ["172.17.23.0/25", "172.17.23.128/25"]
vpc_db_subnets       = ["172.18.23.0/25", "172.18.23.128/25"]
vpc_kuberweb_subnets = ["172.23.0.0/20", "172.23.16.0/20"]
vpc_kuberapi_subnets = ["172.23.64.0/18", "172.23.128.0/18"]
vpc_kuberdb_subnets  = ["172.23.32.0/20", "172.23.48.0/20"]
vpc_dmz_subnets      = ["172.19.23.0/27", "172.19.23.32/27"]
vpc_pep_subnets      = ["172.19.255.0/26", "172.19.255.64/26"]
vpc_netman_subnets   = ["172.19.223.0/27", "172.19.223.32/27"]


# --------
# FORTINET
# --------

//fortinet_password = ""
fortinet_public_keypair = "ssh-rsa "


# -------------------
# VPC PEERING DETAILS
# -------------------

shared_services_1_acceptor_profile       = "networking"
shared_services_1_acceptor_peering_name  = "vpc-peering"
shared_services_1_acceptor_region        = "us-east-1"
shared_services_1_acceptor_account_id    = "934898517106"
shared_services_1_acceptor_role_arn      = "arn:aws:iam::934898517106:role/Admin"
shared_services_1_acceptor_session_name  = "peer"
shared_services_1_acceptor_session_token = ""
